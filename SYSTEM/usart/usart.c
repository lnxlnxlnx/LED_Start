...
/* ── 环形接收缓冲 ── */
volatile u8  g_rx_ring[RX_RING_LEN];
volatile u16 g_rx_ring_wr = 0;
volatile u16 g_rx_ring_rd = 0;
volatile u8  g_rx_ring_has_data = 0;
...
void USART1_IRQHandler(void) // 串口1中断服务程序
{
	...
	if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) // 接收中断
	{
		...
		/* ── 同时写入环形缓冲 (绕过协议) ── */
		_ring_next = (g_rx_ring_wr + 1) & (RX_RING_LEN - 1);
		if (_ring_next != g_rx_ring_rd)
		{
			g_rx_ring[g_rx_ring_wr] = Res;
			g_rx_ring_wr = _ring_next;
			g_rx_ring_has_data = 1;
		}
