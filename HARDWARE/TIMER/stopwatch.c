...
static void sw_ProcessKey(u8 idx)       // 根据处理好的按键索引执行动作(好比上面是按键的时序逻辑，这里是功能更新的组合逻辑---状态机的三段式)
{
    ...
    case K_IDX_KEY0:
    {
        ...
        /*
         * 数据流说明：
         *   PC 发来的每个字节 → USART ISR 写入 g_rx_ring[]（环形缓冲）
         *   KEY0 首次按下 → 从环形缓冲读出到 g_last_rx[]，并标记 g_has_saved = 1
         *   之后 KEY0 再按 → 环形缓冲可能已经空了（上次已读完），
         *                    但 g_has_saved 仍为 1，所以直接发 g_last_rx[] 里的内容
         *   PC 又发新数据 → 环形缓冲又有内容，再次读出并更新 g_last_rx[]
         *
         * 效果：只要 PC 发过一次数据，KEY0 就永远发那次的数据，
         *       直到 PC 再发新数据才会更新。
         */

        /* ── 从环形缓冲读新数据（若有），更新 g_last_rx ── */
        __disable_irq();                        // 读取共享变量时关中断，防止 USART ISR 写冲突
        if (g_rx_ring_has_data)
        {
            ...
            if (g_rx_ring_rd == g_rx_ring_wr)   // 读写指针重合 → 缓冲区已空
                g_rx_ring_has_data = 0;
            /* cnt 一定 > 0，因为 has_data=1 意味着至少写了一字节 */
            g_last_rx_len = cnt;
            g_has_saved = 1;
        }
        __enable_irq();
        ...
        /* ── 决定发什么 ──
         *  g_has_saved = 0 → 从未收到过 PC 数据 → 发默认 SendBuff
         *  g_has_saved = 1 → 收到了 → 发 g_last_rx[]（最近一次 PC 数据）
         */
        if (g_has_saved)
        {
            u16 i;
            for (i = 0; i < g_last_rx_len; i++)
                SendBuff[i] = g_last_rx[i];
            DMA1_MEM_LEN = g_last_rx_len;
            printf("\r\nRX DATA(%u):\r\n", g_last_rx_len);
        }
        else
        {
